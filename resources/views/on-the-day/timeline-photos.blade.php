<div class="row story-row">

    <div class="col-sm-12 col-md-offset-1 col-md-4 text-center">
        <div class="photo-item frame-border animation delay1 fadeInLeft animated">
            <img src="/images/photos/photos.jpg" alt="">
        </div>
    </div>

    <div class="col-sm-12 col-md-2 text-center story-date-wrapper animation fadeIn animated">
        <div class="arrow-left"></div>
        <div class="story-date">
            <div class="date-only">14:30</div>
        </div>
        <div class="arrow-right"></div>
        <div class="clearboth"></div>
    </div>

    <div class="col-sm-12 col-md-4 animation delay1 fadeInRight animated">
        <h3>CHEEKY PHOTOS</h3>
        <p>We're going to take some cheeky photos in the garden (provided the weather is nice). Hang out for a glass of something or head straight to the Duke.</p>
    </div>

    <div class="vertical-line">
    </div>

</div>